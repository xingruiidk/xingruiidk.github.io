...
const maxLives = 3;
const items = [
  { name: 'mcspicy', src: 'image/mcspicy.png', x: canvas.width * 0.65, y: canvas.height * 0.75, width:  canvas.height * 0.2, height: canvas.height * 0.2 },
  { name: 'fries', src: 'image/fries.png', x: canvas.width * 0.85, y: canvas.height * 0.75, width: canvas.height * 0.2, height: canvas.height * 0.2 },
  { name: 'drink', src: 'image/drink.png', x: canvas.width * 0.65, y: canvas.height * 0.5, width: canvas.height * 0.2, height: canvas.height * 0.2 },
  { name: 'icecream', src: 'image/icecream.png', x: canvas.width * 0.85, y: canvas.height * 0.5, width: canvas.height * 0.2, height: canvas.height * 0.2 },
];
...
function drawLives() {
    const heartSpacing = canvas.width * 0.07; // Space between hearts
    ...
    for (let i = 0; i < maxLives; i++) {
        ...
        ctx.drawImage(heartImage, heartX, canvas.height * 0.06  , canvas.height * 0.085, canvas.height * 0.1); // Adjust size and position as needed
    }
}
...
function drawItems() {
  ...
  const bgWidth = (backgroundImage.width/backgroundImage.height) * canvas.height
  ctx.drawImage(backgroundImage, 0, 0, bgWidth, canvas.height);

  if (currentCustomer.image.complete && currentCustomer.image.naturalHeight !== 0) {
    ctx.drawImage(currentCustomer.image, canvas.width * 0.35, canvas.height * 0.25 + currentCustomer.heightOffset, currentCustomer.imageWidth, currentCustomer.imageHeight);
  } else {
  ...
  trayItems.forEach((item, index) => {
    ...
    let trayX = (col * canvas.width * 0.1) + 300;
    let trayY = (row * canvas.height * 0.15) + 500;
